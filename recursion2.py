...
#  Description: More recursion programs
    ...
    if start >= len(nums):
        ...
        for i in range(start + 1, len(nums)):
            if nums[i] == nums[start]:
                b += nums[i]
                counter += 1
        return groupSumClump(start + counter, nums, target - b) or groupSumClump(start + counter, nums, target)
...
def split53Helper(a,b,idx):
    if idx == len(a):
        ...
        if a[idx] % 5 == 0:
            ...
            return splitArrayHelper(a,b,idx + 1)
